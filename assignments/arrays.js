...
console.log(`  The make and model of the last vehicle in inventory is ${inventory[lastCar].car_make}, ${inventory[lastCar].car_model}`);
...
for(let i = 0; i< inventory.length; i++){
    ...
    {
        oldCars.unshift(inventory[i].car_year);
    }
}
oldCars.sort();
console.log(oldCars); 
...
let BMWAndAudi = [];
for(let i = 0; i< inventory.length; i++){
    if(inventory[i].car_make === "BMW" || inventory[i].car_moddel)
    {
        BMWAndAudi.unshift(JSON.stringify(inventory[i]));
    }
}
